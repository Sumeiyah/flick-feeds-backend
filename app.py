...
# CORS: allow localhost React and deployed frontend
CORS(
    app,
...
# 🔧 IMPORTANT CHANGE:
# Remove JWT requirement for now so profiles work without 401.
...
# 🔧 NEW: user search route used by the frontend
...
def post_movie():
    ...
    review = data['Review']
    rating = data['Rating']
    image_path = data['ImagePath'] if data['ImagePath'] else \
...
def user_followers(username):
    ...
    followers_count = db.session.query(
        func.count(Follow.FollowerID)
    ).filter(Follow.FolloweeID == user.UserID).scalar()
...
def user_following(username):
    ...
    following_count = db.session.query(
        func.count(Follow.FolloweeID)
    ).filter(Follow.FollowerID == user.UserID).scalar()
